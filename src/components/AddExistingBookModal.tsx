...
export function AddExistingBookModal({ series, books, onClose, onSuccess }: AddExistingBookModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-slate-800">
          <div>
            <h2 className="text-xl font-black text-slate-100 tracking-tight">Add to Series</h2>
            <p className="text-sm font-bold text-slate-400 mt-1 uppercase tracking-widest">{series}</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-4 border-b border-slate-800 bg-slate-950/50">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-slate-900 border-2 border-slate-700 rounded-xl focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-medium text-slate-200 shadow-sm"
            />
          </div>
        </div>
          ...
          {filteredBooks.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredBooks.map((book) => (
                <div key={book.id} className="flex flex-col sm:flex-row gap-4 p-3 rounded-2xl border border-slate-800 hover:border-indigo-100 hover:shadow-md transition-all bg-slate-900 group items-center">
                  {book.cover ? (
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    <p className="text-xs text-slate-400 mt-1 uppercase tracking-wider font-semibold line-clamp-1">{book.author}</p>
                    {book.series && (
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-slate-800 text-indigo-700 font-black uppercase tracking-widest text-xs hover:bg-indigo-600 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed group-hover:bg-indigo-50"
                  >
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950 flex justify-end">
          <button
            onClick={onClose}
            className="px-6 py-3 bg-slate-900 border-2 border-slate-700 text-slate-200 rounded-xl font-bold hover:bg-slate-950 transition-colors"
          >
            Done
          </button>
        </div>
